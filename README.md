...
- The UI exposes lifetime evaluation/Buy/Sell/Hold/failure counters, today’s provider calls and cost, the last signal source, pass state and budget status. Confirmed activity is authoritative for Buy/Sell totals, so an older strategy cannot lose its fills when its detailed decision window changes. New evaluations are stored in an append-only per-vault journal; the latest 100 also remain embedded as an outage fallback. The UI and **Export CSV activity** return every available journal and on-chain activity row. Strategies created before the complete journal may have an explicitly labelled gap in old Hold/failure detail, but their surviving counters and confirmed fills remain visible. Raw provider text stays behind Technical error details. When xAI returns `usage.cost_in_usd_ticks`, PULSE records that exact provider-billed amount; token-rate calculation is only the fallback for compatible responses without billed-cost ticks.
