...
export const SpotGeneratedAnalysisSchema = z.object({
  ...
  elliottWave: z.object({
    degree: z.enum(["primary", "intermediate", "minor", "unclear"]),
    structure: z.enum(["impulse", "diagonal", "abc_correction", "complex_correction", "unclear"]),
    direction: z.enum(["up", "down", "unclear"]),
    currentWave: z.enum(["1", "2", "3", "4", "5", "A", "B", "C", "unclear"]),
    confidence: z.number().min(0).max(100),
    rationale: z.string(),
    invalidation: z.number().nullable(),
    paths: z.array(z.object({
      type: z.enum(["wave_3_continuation", "wave_5_continuation", "abc_correction", "wave_c_continuation", "count_invalidation", "recount"]),
      label: z.string(),
      thesis: z.string(),
