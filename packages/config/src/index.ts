...
export function loadConfig(): AppConfig {
  ...
  const routes: RoutePriceMap = {
    ...
    "POST /v1/token/scan": {
      name: "Token Risk Report (legacy alias)",
      priceUsd: parsed.PRICE_TOKEN_SCAN,
      description: "Compatibility alias for the paid multi-source Token Risk report.",
    },
    "POST /v1/preflight": {
      name: "Token Risk Guard",
      priceUsd: parsed.PRICE_PREFLIGHT,
      description: "A Grok-synthesized token due-diligence report using live contract, market, holder, project website, social-profile and promotion evidence. Raw contract evidence and exact transaction simulation remain free.",
    },
    "POST /v1/wallet/scan": {
  ...
  return {
    ...
    registration: {
      ...
      fields: {
        name: cfg.productName,
        description: cfg.productDescription,
        priceHint: "Token Risk Guard $0.20 · Quick reports $0.20 · Pro reports $0.30 · multichain x402 settlement",
        endpoint: `${base}/mcp`,
        logo: cfg.logoUrl,
      },
    },
